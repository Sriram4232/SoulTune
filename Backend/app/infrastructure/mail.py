"""Email OTP delivery using SMTP or Resend."""
...
def send_otp(config: Settings, recipient: str, code: str) -> None:
    ...
    try:
        import sys
        mail_mod = sys.modules.get("app.mail")
        _httpx = getattr(mail_mod, "httpx", httpx) if mail_mod else httpx
        _smtplib = getattr(mail_mod, "smtplib", smtplib) if mail_mod else smtplib
        ...
        if config.mail_provider == "resend":
            ...
            if config.smtp_security == "ssl":
                connection = _smtplib.SMTP_SSL(config.smtp_host, config.smtp_port, timeout=10, context=context)
            else:
                connection = _smtplib.SMTP(config.smtp_host, config.smtp_port, timeout=10)
            with connection:
